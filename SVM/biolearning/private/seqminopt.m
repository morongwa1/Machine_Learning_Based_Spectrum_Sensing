...
% SEQMINOPT Sequential Minimal Optimization for Support Vector Machines
%
% This function implements a sequential minimization algorithm
% to solve the optimization problem arising in support vector
% machine learning. The basic idea [1] is to pick a working set of
% just two alphas and solve the optimization for these two alphas
% while keeping all other variables fixed. This is done in the function
% 'updateAlphas' below. The working set selection is done according to the
% 'maximum gain' method [2] and is contained in the main loop below.
% The third element of the algorithm consists of a caching strategy for
% the kernel matrix for medium sized and large problems (when the kernel
% matrix is too large to fit into memory) in order to keep the number of
% kernel evaluations minimal.
% A further property is the ability of the algorithm to stop even if
% not all KKT conditions are satisfied. This can be specified in the
% options struct which contains a field with the fraction of points that
% are allowed to violate KKT. This is useful for big, noisy problems,
% where the algorithm sometimes spends a lot of time fluctuating very
% near the optimal solution.
% Note that this algorithm doesn't implement shrinking which is a heuristic
% method useful for speeding up the algorithm for large scale problems.
% This could be a further future improvement.
%
% Input - Arguments:
% data: the basic input data points as a two dimensional matrix. Each
% row corresponds to one data point (called x_i in the paper).
% targetLabels: a column vector of labels. +1 or -1 for each data point
% according to which group the data point belongs to. Two groups must be
% present.
% boxConstraints: A column vector of numbers, each of which must be
% larger than the tolerance with which the KKT conditions are checked.
% kernelFunc: the kernel function that defines the optimization problem.
% smoOptions: optional struct with a number of options. See the function
% CREATESMOOPTIONS for details.
%
% Return Value:
% alphas: A column vector with optimized Lagrange multipliers, one for
% each data point. Each alpha_i is guaranteed to be within [0, C_i] where
% C_i is the boxconstraint for the data point with index i.
% offset: also known as bias.
%
% References:
...
%   $Revision: 1.1.6.7 $  $Date: 2008/03/28 15:21:43 $
...
if length(size(data)) ~= 2
    error('Bioinfo:seqMinOpt:InvalidInput', ...
        'data object must be two-dimensional');
end
nPoints = size(data, 1);
...
if ~isequal(size(targetLabels), [nPoints 1])
    error('Bioinfo:seqMinOpt:InvalidInput', ...
        'targetLabels size does not match to data object');
end
% both groups must be present, otherwise the linear equality
% constraint enforces alphas = 0!
% Groups must be coded with +-1
if ~isequal(sort(unique(targetLabels)), [-1;1])
    error('Bioinfo:seqMinOpt:InvalidInput', ...
        'targetLabels must contain exactly two groups coded by +1 and -1');
end

% boxConstraint: a column vector of positive numbers
if ~isequal(size(boxConstraints), size(targetLabels))
    error('Bioinfo:seqMinOpt:InvalidInput', ...
        'boxConstraints must have same size as targetLabels');
end
if ~all(boxConstraints >= smoOptions.TolKKT)
    error('Bioinfo:seqMinOpt:InvalidInput', ...
        'all boxConstraints must be greater than the tolerance');
end

% pass over to implementation function:
[alphas offset] = seqminoptImpl(data, targetLabels, ...
    boxConstraints, kernelFunc, ...
    smoOptions);
...
for i = 1:length(alphas)
    if ~( (alphas(i) >= -eps) && (alphas(i) <= boxConstraints(i) + eps))
        error('Bioinfo:seqMinOpt:ViolatedPostcondition', ...
            'Boxconstraint Violation for index %d, alpha = %f, boxConstraint = ', ...
...
if abs(linEqConstraint) > smoOptions.TolKKT
    error('Bioinfo:seqMinOpt:ViolatedPostcondition', ...
        'Violated Lin-Eq. Constraint: %f', linEqConstraint);
end
end % end of seqMinOpt
...
smoStartTime = cputime;

% Working copy of tolerance by which the KKT conditions and stopping
% criteria are checked
tolKKT = smoOptions.TolKKT;

% Tolerance by which support vectors are identified
% (compare svmtrain function):
svTol = sqrt(eps);
...
error('Bioinfo:seqMinOpt:NoConvergence', ...
    'No convergence achieved within maximum number (%i) of main loop passes', ...
    smoOptions.MaxIter);
...
% -------------------------------------
    ...
    function [val2 idx2] = getMaxGain(val1, idx1)
        ...
        % idx2 = NaN.
        [maxGain, idx2] = max(gainNumerator ./ gainDenominator);
        tmp = idxHelper(mask);
        idx2 = tmp(idx2);
    ...
    function flags = checkKKT()
        ...
        flags = repmat(false, size(amount));
    ...
    function reportStatus(numIter, stopCrit)
        ...
        disp('seqMinOpt status:');
        disp(sprintf('Number of iterations: %i', numIter));
        disp(sprintf('Execution time: %f seconds', cputime - smoStartTime));
        disp(sprintf('Value of stopping criterion: %f', stopCrit));
        disp(sprintf('Value of objective function: %f', objFunc));
        disp(sprintf('All SVs: %d', sum(alphas > svTol)));
        nTrueSvs = sum( (alphas > svTol) & (alphas < (boxConstraints - svTol)));
        disp(sprintf('Free SVs: %d', nTrueSvs));
        if isempty(fullKernel)
            disp(sprintf('Number of cached kernel columns: %i (of %i possible columns)', ...

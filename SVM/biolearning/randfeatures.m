...
%   RANKFEATURES, SEQUENTIALFS, SVMCLASSIFY. 
 
%   Copyright 2003-2008 The MathWorks, Inc.
%   $Revision: 1.1.10.8 $  $Date: 2008/06/16 16:32:43 $
...
if ~isnumeric(X) || ~isreal(X)
   error('Bioinfo:randfeatures:NotNumericAndReal',...
         'The observation vectors must be numeric an real.') 
end

if numel(group) ~= numSamples
   error('Bioinfo:randfeatures:NotEqualNumberOfClassLabels',...
...
if nvarargin
    if rem(nvarargin,2)
        error('Bioinfo:randfeatures:IncorrectNumberOfArguments',...
    ...
    for j=1:2:nvarargin
        ...
        if isempty(k)
            error('Bioinfo:randfeatures:UnknownParameterName',...
                'Unknown parameter name: %s.',pname);
        elseif length(k)>1
            error('Bioinfo:randfeatures:AmbiguousParameterName',...
                'Ambiguous parameter name: %s.',pname);
        else
            switch(k)
                case 1 % classifiers
                    classifiers = {'knn','da'};
                    classifier = strmatch(lower(pval),classifiers); 
                    if isempty(classifier) 
                      error('Bioinfo:randfeatures:NotValidClassifier','Not a valid classifier.')
                    end
                ...
                case 3 % performance threshold
                    ...
                    if performanceThreshold <0 || performanceThreshold >1
                        error('Bioinfo:randfeatures:NotValidThreshold','Performance threshold must be a value between 0 and 1')
                    end
                case 4 % confidence threshold
                    confidenceThreshold  = pval(1);
                    if confidenceThreshold <0 || confidenceThreshold >1
                        error('Bioinfo:randfeatures:NotValidThreshold','Confidence threshold must be a value between 0 and 1')
                    end
                case 5 % subset size
                ...
                case 7 % Number of indices
                    ...
                    if ~isnumeric(numIndices) || numIndices<1 || numIndices>numPoints
                        error('Bioinfo:randfeatures:NotValidN','Number of output indices must be a scalar between 1 and number of features.')
                    end
                case 8 % cross normalization
                    cnorms = {'none','meanvar','softmax','minmax'};
                    cnorm = strmatch(lower(pval),cnorms); 
                    if isempty(cnorm) 
                      error('Bioinfo:randfeatures:NotValidNormalization','Not a valid cross-normalization.')
                    end
                    cnorm = cnorms{cnorm};
                case 9 % verbose
                    verbose = opttf(pval,okargs{k},mfilename);
            end
        end
    end
end
...
if min(accumarray(group,1))<1 || numGroups<2
    error('Bioinfo:randfeatures:MissingObservations','There must be at least one observation for every group and more than one group.')
end
...
while (numGood < poolSize)
    ...
    thePerm = ceil(rand(1,subsetSize)*numPoints);
    % check that the sample contains unique data points
    if numel(unique(thePerm))<subsetSize
        continue
    end
    ...
    switch classifier
        ...
        otherwise % linear or quadratic
            [c,err,pos] = classify(X(thePerm,:)',X(thePerm,:)',group,daOptions{:});
    ...
    if classPerformance>=performanceThreshold
        ...
        if verbose
            disp(sprintf('Tried %d subsets, accepted %d.',numTried,numGood));
        end
    end
end
if verbose 
...
[vals,idx] = sort(z,1,'descend');
idx = idx(1:numIndices);

% Check that features were found at least once, if not trim idx
if z(idx(end))==0
   idx = idx(z(idx)>0);
   warning('Bioinfo:randfeatures:NotEnoughPoints',...
      'Only %d significant features could be found.',numel(idx))
end

...
%SVMSMOSET Create/alter SMO OPTIONS structure for use in SVMTRAIN.
%
...
%   violate the KKT conditions. Settng this to a value greater than 0 will
...
%
% See also OPTIMSET, SVMCLASSIFY, SVMTRAIN.
...
%   $Revision: 1.1.6.4 $  $Date: 2007/07/23 21:21:58 $
...
while i <= nargin
    ...
    if ~isempty(arg)                      % [] is a valid options argument
        if ~isa(arg,'struct')
            error('Bioinfo:svmsmoset:NoPropNameOrStruct',...
...
if rem(nargin-i+1,2) ~= 0
    error('Bioinfo:svmsmoset:ArgNameValueMismatch',...
...
if expectval
    error('Bioinfo:svmsmoset:NoValueForProp',...
